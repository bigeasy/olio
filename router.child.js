...
/*
    ...
    usage: olio <socket> router <options> <target>

    options:

        -l, --listeners <number>

            Count of HTTP listeners.

        -b, --bind <interface:port>

            Interface and port to listen on. Interface is optional. If not
            specified all interfaces are bound for the given port.

        -k, --key <key>

            Evaluated as a JavaScript function that will extract the key
            material used to select a worker.

        -b, --broadcast <METHOD /url>

            Specify a path that will be broadcast to all workers.
