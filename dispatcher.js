...
function Dispatcher (destructible, transmitter, callback) {
    ...
    var messages = this.messages.parent.pump(this, 'dispatch', destructible.monitor('messages'))
    destructible.destruct.wait(messages, 'destroy')
    this.destructible = destructible
    this.siblings = new Cubbyhole
    this.transmitter = transmitter
    this.transmitter.register(this.messages)
}
...
Dispatcher.prototype.dispatch = cadence(function (async, envelope) {
    ...
    case 'initialize':
        this._ready.unlatch(null, this._createBinder(this, message))
        break
    case 'connect':
